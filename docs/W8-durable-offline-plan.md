...
| (d′) boot trigger FIRES once on a real boot → replay on connect | ✅ PASS (real `adb reboot`, full round-trip) | after reboot: `boot_count` 3→4, app cold-started via `BootReceiver`, `boot_generation` stamped 3→4 (generation-gated single fire), one `trigger.fired` committed to the durable queue with NO session; then launching `MainActivity` (boot brings up the headless firing service via `EbpApplication` but NOT the server socket — that binds with the activity) and connecting drained it: `:delivered 1 :remaining 0`, queue → 0. (`am broadcast BOOT_COMPLETED` can't drive this — protected broadcast, shell denied — so a real reboot was required. WiFi-adb does not survive the reboot and drops on screen-lock; reconnect needs unlock + `adb connect IP:NEWPORT`.) |
