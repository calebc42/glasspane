...
5. **OR-2 SUPERSEDED (Caleb, 2026-08-31):** the component gallery remains an org-free Jetpacs facility, but EF Themes is a Glasspane package opinion again. `glasspane-ef.el` owns its actions and lifecycle under owner `glasspane` and contributes its row through Jetpacs' neutral Modus-family Theme Settings registry. Jetpacs names neither EF nor Glasspane.
...
### GR-8d — periphery re-homing (**EF disposition superseded 2026-08-31**)
GR-7a removed EF's private Glasspane couplings, but that did not make its package choice Jetpacs policy. EF now lives in `glasspane-ef.el`, registers under owner `glasspane`, and contributes a row to the neutral Modus-family Theme Settings registry. The gallery remains at `emacs/jetpacs-gallery.el` under owner `jetpacs.demo`. SRS stays app-side (first-class Review destination, notes coupling; its funnels are mode-neutral or foundation-fed).
...
| glasspane-ef.el | survives | Glasspane-owned EF package adapter; contributes to the Jetpacs Modus-family Theme Settings registry |
...
**Upstream candidates (12 → disposition):** table.el→GR-10/PA-5 (OR-4 ratified) · save-policy seam→GR-6 · freshness merge→GR-6 · per-heading affordances→GR-10 · mutation verb family→GR-10 · cross-file search→GR-10 · dates util→GR-7b · settings-remove-link→GR-7b · FAB registry→GR-7b · clock→GR-5(OR-1 ratified) · gallery→GR-8d/PA-3d while EF remains Glasspane-owned (OR-2 superseded) · demo seeder→GR-10(OR-3).
