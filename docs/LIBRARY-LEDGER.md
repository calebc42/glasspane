...
| **androidx.sqlite + sqlite-bundled** · `androidx.sqlite:sqlite`, `androidx.sqlite:sqlite-bundled` | 2026-07-31, local clone @ `d69c96e`: driver-level `open(fileName, flags)` with public `SQLITE_OPEN_READONLY`/`SQLITE_OPEN_URI` (`BundledSQLiteDriver.kt:84,:95`; `BundledSQLite.kt:26,:35`); bundles SQLite 3.50.1, `SQLITE_THREADSAFE=2`; **no js/wasmJs targets** (web is `sqlite-web`, whose `open` is `suspend`) | infrastructure | Inside `ebp-sqlite`, behind its declare-schema/apply-changesets interface; costs priced in PLAN-refound RF-4c (`BEGIN IMMEDIATE` helper, thread confinement, no `InvalidationTracker`) | RF-4c |
