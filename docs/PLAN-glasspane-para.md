...
12. **PA-OR-4 PARTLY SUPERSEDED (Caleb, 2026-08-31):** the gallery remains a generic Jetpacs facility under `jetpacs.demo`. EF Themes is Glasspane-owned package policy under owner `glasspane`; `glasspane-ef.el` registers its actions and contributes its row through Jetpacs' neutral Modus-family Theme Settings registry. Only the EF opener is cross-surface; its inner controls remain owner-scoped through sanctioned guest delegation.
...
4. **PA-OR-4 corrected 2026-08-31:** `glasspane-ef.el` remains a Glasspane module and owns EF registration; Jetpacs exposes only the Modus-family provider registry consumed by it. `glasspane-gallery.el` alone moved to `emacs/jetpacs-gallery.el`. The residual audit leaves `ef.show` and `demo.gallery` as cross-surface openers; inner controls are owner-scoped.
...
> **2026-09-15 amendment — membership authority moved to the engine.**  `ebp-org` now owns the tag-group primitive (`ebp-org-tag-groups`, `ebp-org-tag-groups-at`, `ebp-org-heading-tag-groups`).  Glasspane keeps only its PARA opinions: which group (`glasspane-area-tag-group`) and forced inheritance (`glasspane-org-tag-groups-at` binds `org-use-tag-inheritance` to t around the engine call).  The PM-2 substrate survives as the public `glasspane-org-file-tag-group-index`, a per-file memo derived from the engine reader and keyed on the buffer's tag settings as well as the EBP content stamp; T-7's "derive the fold from the memoised index" now points at that public name.  `glasspane-areas.el` and `glasspane-resources.el` read no `glasspane-org--` privates; `glasspane-org-heading-item-at` and `glasspane-org-tag-strings` are public for the same reason.  The detail view's title carries the shared Area chips, its Tags section groups membership by native declaration, and editing routes to the foundation's grouped picker.
...
> **PM-7 GATE COMPLETE 2026-09-04.**  `tools/para-migrate.el` (ships in the managed-updater payload; `tools/` is not excluded).  Desktop command line: `emacs -Q --batch -L ../ebp.el/lisp -L ../ebp-org/lisp -L ../jetpacs-poc/emacs -l tools/para-migrate.el --eval '(glasspane-para-migrate "~/pkb")'` (add `:write t`, `:force t` on a dirty tree, `:scaffold t` for `areas.org`, `:custom-file` for the Customize save).  Evidence: warning-as-error compile and Checkdoc clean; on a scratch copy of the vault, write → write → dry-run yields eight `WROTE` lines, then only `SKIP (already)` lines, with a pre-existing Customize setting preserved beside the new group.  Real vault: dry-run reviewed, then write — `homelab/infrastructure.org:4` (trailing colon), `restic-recovery-guide.org:9` (FILETAGS syntax on a TAGS line), `supernote.org:384` (commas; `e-ink`/`open-source` → `e_ink`/`open_source`), `corpus.org:866,875` (`:AREA:` → `:FIELD_OF_STUDY:`), and the three `:project:` → `:workspace:` filetags.  The Area group was saved through the **live** Emacs session (`customize-save-variable` into `~/.emacs.d/custom.el`), not the batch writer: a `-Q` batch session cannot own the user's custom file.  `~/.emacs.d/config.el` now loads the ELPA vulpea (the dead checkout guard replaced) and the same block was evaluated live: vulpea 20260714.543, autosync on.  Real-vault index (batch, seeded group): career 2 open / 2 files · computing 0 / 0 (outside the non-recursive scope) · meta 0 / 4 · sovereignty 1 / 1 · tech 15 / 2 — equal to the migration report; Projects by file = ideas 1, inbox 3, kmp 2, learning 16, resume_master 2, sovereignty-stack 1; no `_archive` files yet.  **Recorded honestly:** the pre-write backup command failed on a glob quoting mistake and the write ran without it; the eight edits are exact known lines, a reverse script (`pkb-reverse.sh`) and a post-migration tarball were written to the session scratchpad immediately after.  The touched vault files are untracked in the `~/pkb` repository, so no vault commit was possible; no `areas.org` was scaffolded (opt-in, left to Caleb).
...
> **PM-8 DEVICE BATCH COMPLETE 2026-09-04; SOAK PENDING.**  Managed updater run (`onboard-tablet.sh --emacs-home termux --vault shared --skip-pylsp`): 168 `.el` files refreshed, Companion 0.1.0-w4 rebuilt and reinstalled with data preserved, `/sdcard/Jetpacs` untouched; stale `.elc` under the deployed applet dir removed; Emacs and Companion force-stopped and relaunched in PORTRAIT.  The tablet vault is the demo vault (glasspane-demo-*.org + inbox.org) whose files already declare the `Area` group, so `para-migrate.el` had nothing to repair there and the Area Tags settings row already lists House / Auto / Bills / Work / Health / Learning / Digital.  Screenshots (session scratchpad, `shot-NN-*.png`): Glasspane opened as primary with the five-destination rail; Areas list with per-member counts; Digital drill — chips, Area-elevated project cards, Work intersection narrowing the list, Resources rows, Archives; drawer with the six destinations and their new subtitles; Archive screen with Area chips, Health filter, row → Org reader (`glasspane-demo-health.org_archive`, rail intact) → Back to Archive with the filter kept; Projects by file with keyword chips, then By Area; Resources → Files at `/storage/emulated/0/Jetpacs/org/` with Resources selected and the capture FAB; Settings → Glasspane → Area Tags.  **One defect found on hardware and fixed live:** the tablet spells `org-directory` through the `/sdcard` symlink while items carry truenames, so the Archive walk's siblings never matched — the walk now records truenames and the sibling match truenames its inputs (`glasspane-resources--archive-root`, `glasspane-resources-archives-for-files`); pushed over ssh, reloaded through the Eval REPL, Archives populated.  Observed: system Back from the Resources browser re-presents the previous destination (Projects) through the `glasspane.files.return` path rather than resetting to Agenda — the navigation contract's primary return, recorded not fixed.  Areas first paint on the demo vault was immediate (T-7 quiet).  Not exercised: adding an Area through the settings dialog, Agenda-root Back finishing the Activity.  The one daily-driver soak day is Caleb's; `pm-closed` is tagged after it.
